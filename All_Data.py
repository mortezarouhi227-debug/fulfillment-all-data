...
# تاریخ‌خوان برای ستون A تب Other Work (فقط تاریخ)
...
# ---------------------------
# Other Work (حذف افراد از «تاریخِ ستون A به بعد»)
# ---------------------------
...
for sheet_name in source_sheets:
    ...
    for row in values[1:]:
        try:
            ...
            if not record_date or not hour:
                continue
            ...
            if sheet_name == "Pack":
                order_val = float(order) if order else 0
                if order_val > 0:
            ...
            new_rows.append([
                ...
                hour, occupied, order, perf_without, perf_with, neg_min,
                ipo_pack, user, shift
            ])
        except Exception as e:
